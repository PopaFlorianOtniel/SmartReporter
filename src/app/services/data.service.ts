...
import { addDoc, collection, collectionData, deleteDoc, doc, docData,
Firestore, setDoc, updateDoc } from '@angular/fire/firestore';
...
export class DataService {
  ...
  getReports(): Observable<Report[]> {
    ...
    return collectionData(reportsRef, { idField: 'id'}) as
      Observable<Report[]>;
  }

  getReportById(id:any): Observable<Report> {
    const reportDocRef = doc(this.firestore, `reports/${id}`);
    return docData(reportDocRef, { idField: 'id' }) as
      Observable<Report>;
  }
  ...
  deleteReport(report: Report) {
    const reportDocRef = doc(this.firestore,
      `reports/${report.id}`);
  ...
  updateReport(report: Report) {
    const reportDocRef = doc(this.firestore,
      `reports/${report.id}`);
    return updateDoc(reportDocRef, { title: report.title,
  ...
  async uploadImage(cameraFile: Photo, imageName: string) {
    ...
    try {
      await uploadString(storageRef, cameraFile.base64String as
        string, 'base64');
  ...
  async setProfileImage(cameraFile: Photo) {
    const user = this.auth.currentUser;
    try {
      const imageUrl = await this.uploadImage(cameraFile,
        'profile.png');
      const userDocRef = doc(this.firestore, `users/${user!.uid}`);
